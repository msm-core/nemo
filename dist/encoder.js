...
class HDVEncoder {
    ...
    _rng;
    _atoms = new Map();
    constructor(dim = hdc_1.DIM, seed = SEED) {
        this.dim = dim;
        this._rng = (0, hdc_1.makeRNG)(seed);
    ...
    _atom(key) {
        if (!this._atoms.has(key)) {
            this._atoms.set(key, (0, hdc_1.randomHV)(this.dim, this._rng));
        }
    ...
    encode(tokens) {
        ...
        while (i < tokens.length) {
            ...
            if (ttype === "CONCEPT" && tok.field && FIELD_SET.has(tok.field)) {
                ...
                if (i + 1 < tokens.length && tokens[i + 1].type === "ROLE" && tokens[i + 1].role) {
                    ...
                    content.push((0, hdc_1.bind)(cHV, rHV), cHV, rHV);
                    i += 2;
                    continue;
                }
                content.push(cHV);
        ...
        if (content.length === 0) {
            return [(0, hdc_1.randomHV)(this.dim, this._rng), null];
        }
        let hv = (0, hdc_1.bundle)(content);
        if (strMods.length > 0)
            hv = (0, hdc_1.bundle)([hv, ...strMods]);
        for (let j = strOps.length - 1; j >= 0; j--) {
            hv = (0, hdc_1.bind)(strOps[j], hv);
        }
        let dominant = null;
        let maxCount = 0;
