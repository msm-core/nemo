...
exports.HDCAgent = void 0;
const hdc_1 = require("./hdc");
class HDCAgent {
    dim;
    ...
    observe(hv, field) {
        ...
        if (proto) {
            const s = (0, hdc_1.similarity)(hv, proto);
    ...
    classify(hv) {
        ...
        for (const [f, p] of this._proto)
            scored.push([f, (0, hdc_1.similarity)(hv, p)]);
    ...
    verify(hv, field) {
        ...
        if (!proto)
            return [false, 0];
        const sim = (0, hdc_1.similarity)(hv, proto);
    ...
    feedback(hv, confirmedField, meta = {}) {
        ...
        const sim = proto ? (0, hdc_1.similarity)(hv, proto) : 1.0;
    ...
    retrieve(queryHV, k = 5, field) {
        ...
        if (field) {
            const eps = this._episodes.get(field) ?? [];
            return [...eps]
                .sort((a, b) => (0, hdc_1.similarity)(queryHV, new Float32Array(b.hv)) -
                (0, hdc_1.similarity)(queryHV, new Float32Array(a.hv)))
                .slice(0, k);
        }
        const protoRanked = [...this._proto.entries()]
            .filter(([f]) => this._episodes.has(f))
            .map(([f, p]) => [f, (0, hdc_1.similarity)(queryHV, p)])
            .sort((a, b) => b[1] - a[1])
            .slice(0, k);
        return protoRanked.map(([f]) => {
            const eps = this._episodes.get(f);
            return eps.reduce((best, e) => (0, hdc_1.similarity)(queryHV, new Float32Array(e.hv)) >
                (0, hdc_1.similarity)(queryHV, new Float32Array(best.hv))
                ? e
                : best);
        });
    }
