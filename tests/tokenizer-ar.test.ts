...
describe("CST Arabic Tokenizer", () => {
  ...
  test("basic CONCEPT from root map — علم (know)", () => {
    ...
    expect(toks.some((t) => t.type === "CONCEPT" && t.field === "know")).toBe(
      true,
    );
  });
  ...
  test("CONCEPT from direct field map — animal (كلب)", () => {
    ...
    expect(toks.some((t) => t.type === "CONCEPT" && t.field === "animal")).toBe(
      true,
    );
  });
  ...
  test("CONCEPT from direct field map — color (أحمر)", () => {
    ...
    expect(toks.some((t) => t.type === "CONCEPT" && t.field === "color")).toBe(
      true,
    );
  });
  ...
  test("CONCEPT from direct field map — body (قلب)", () => {
    ...
    expect(toks.some((t) => t.type === "CONCEPT" && t.field === "body")).toBe(
      true,
    );
  });
  ...
  test("handles diacritics in input", () => {
    ...
    expect(toks.some((t) => t.type === "CONCEPT" && t.field === "write")).toBe(
      true,
    );
  });
