...
export function randomHV(dim: number = DIM, rng?: () => number): Float32Array {
...
export function bundle(vectors: Float32Array[]): Float32Array {
  if (vectors.length === 0) throw new Error("bundle: empty array");
  ...
  for (let i = 0; i < dim; i++) out[i] = sum[i] >= 0 ? 1 : -1;
  return out;
}
...
export function nearest(
  query: Float32Array,
  memory: Map<string, Float32Array>,
): Array<[string, number]> {
