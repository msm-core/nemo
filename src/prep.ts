...
export function buildFrame(text: string, tokens: NemoToken[]): ReasoningFrame {
  ...
  // Creative override
  if (
    [...CREATIVE_TRIGGERS].some((w) => textLower.includes(w)) &&
    !fieldOverride
  ) {
  ...
  if (hasNegation) {
    for (let i = 0; i < tokens.length; i++) {
      if (tokens[i].type === "NEG") {
        ...
        if (next?.type === "CONCEPT" && next.field)
          negatedFields.push(next.field);
      }
    }
  }
